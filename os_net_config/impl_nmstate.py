...
class NmstateNetConfig(os_net_config.NetConfig):
    ...
    def is_device_managed(self, net_device):
        ...
        if net_device.remove_type in special_handlers:
            ...
            if net_device.remove_type == "sriov_vf" and result is None:
        ...
        if not device_state or \
            device_state.get(Interface.STATE) == InterfaceState.IGNORE:
    ...
    def _check_ovs_bridge(self, net_device):
        ...
        if not ovs_bridge_cfg:
            return False
        if ovs_bridge_cfg.get(Interface.STATE) == InterfaceState.IGNORE:
            return False
    ...
    def _check_ovs_bond(self, net_device):
        ...
        for bridge_cfg in all_bridges:
            if bridge_cfg.get(Interface.STATE) == InterfaceState.IGNORE:
                continue
