...
DISPATCH = 'dispatch'
MNG_RULES = '/var/lib/os-net-config/nmstate_files/rules.yaml'
...
class NmstateNetConfig(os_net_config.NetConfig):
    ...
    def _is_managed_iprules(self, c_rule):
        ...
        if os.path.exists(MNG_RULES):
            mng_iprules = yaml.safe_load(common.get_file_data(MNG_RULES))
            filtered_c_rule = {
    ...
    def apply(self, cleanup=False, activate=True, config_rules_dns=True):
        ...
        if activate:
            ...
            try:
                if rules_applied:
                    common.write_yaml_config(MNG_RULES, rules_applied)
            except NameError:
                common.write_yaml_config(MNG_RULES, rules_template)
