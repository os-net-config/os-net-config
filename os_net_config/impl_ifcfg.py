...
class IfcfgNetConfig(os_net_config.NetConfig):
                ...
                for line in f:
                    ...
                    if line.startswith('DEVICE='):
                        ...
                        bond_members = (bond_ifaces_value
                                        .replace(',', ' ').split())
                        if net_device.dev_name in bond_members:
                    ...
                    if (bond_ifaces_found and is_dpdk_bond and
                        ...
                        logger.warning(
                            ...
                            net_device.dev_name, device_name,
                            net_device.dev_name)
    ...
    def _check_single_ifcfg_file(self, net_device, ifcfg_file):
        ...
        :param is_ovs_dev: Boolean indicating if this is an OVS device type
        ...
        is_ovs_dev = net_device.dev_type in ovs_type_map
                ...
                for line in f:
                    ...
                    if line.startswith('DEVICE='):
                        device_value = (line.split('=', 1)[1]
                                        .strip().strip('"\''))
                        if device_value == net_device.dev_name:
                            device_found = True
                        else:
                            return False
    ...
    def _handle_sriov_vf_device(self, net_device):
        ...
        and the dpdk device is managed by ifcfg provider, then the VF shall
        also be listed as managed by ifcfg provider.
        ...
        if vf_pci_address is not None:
            ...
            return self._check_dpdk_files_for_vf_pci_address(
                net_device, vf_name, vf_pci_address)
    ...
    def _check_dpdk_files_for_vf_pci_address(self, net_device, vf_name,
                                             vf_pci_address):
        """Check ifcfg files for DPDK ports/bonds matching VF's PCI address.

        :param net_device: RemoveNetDevice object for the SR-IOV VF
        :param vf_name: Name of the VF to match
        :param vf_pci_address: PCI address of the VF to match
            ...
            for filename in os.listdir(ifcfg_dir):
                if filename.startswith('ifcfg-'):
                    ...
                    if self._is_dpdk_config_file(ifcfg_file):
                        ...
                        # Check if VF's PCI address matches any DPDK PCI
                        # address
                        if vf_pci_address in dpdk_pci_addresses:
                            logger.debug("%s: Found VF PCI address %s in DPDK "
                                         "config %s. Please remove the DPDK "
                                         "port %s", net_device.dev_name,
                                         vf_pci_address, ifcfg_file, vf_name)
                            return True
        except (IOError, OSError) as e:
    ...
    def _check_ifcfg_file_for_ovs_dev(self, net_device, ifcfg_file):
        ...
        if net_device.dev_type == "ovs_dpdk_port":
        ...
        with open(ifcfg_file, 'r') as f:
            for line in f:
                ...
                if line.startswith('DEVICE='):
                    ...
                    file_dev_type = (line.split('=', 1)[1]
                                     .strip().strip('"\''))
                    if file_dev_type == type_map.get(net_device.dev_type, ""):
            ...
            if (device_found and ovs_dev_found and type_found):
                if net_device.dev_type in ["ovs_dpdk_port",
                                           "ovs_dpdk_bond"]:
    ...
    def is_device_managed(self, net_device):
        ...
        if not os.path.exists(ifcfg_dir):
            logger.debug("ifcfg directory %s does not exist, hence ruling out "
                         ...
                         ifcfg_dir, net_device.dev_name)
            return self._device_managed_status(net_device, False)

        if net_device.dev_type == "sriov_vf":
            rv = self._handle_sriov_vf_device(net_device)
        ...
        try:
            ...
            likely_filename = f"ifcfg-{net_device.dev_name}"
