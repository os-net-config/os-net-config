...
class TestSriovConfig(base.TestCase):
    ...
    def test_del_udev_rule_for_legacy_sriov_pf_file_not_exists(self):
        # Test when udev file doesn't exist
        sriov_config.del_udev_rule_for_legacy_sriov_pf('eth1')
        # Should not raise any exception

    def test_del_udev_rule_for_legacy_sriov_pf_remove_rule(self):
        def fake_reload_udev():
            pass
        self.stub_out('os_net_config.sriov_config.reload_udev_rules',
                      fake_reload_udev)

        # Create a udev file with rules
        udev_content = '''# Comment line
KERNEL=="eth1", RUN+="/bin/os-net-config-sriov -n %k:5"
KERNEL=="eth2", RUN+="/bin/os-net-config-sriov -n %k:3"
'''
        with open(sriov_config._UDEV_LEGACY_RULE_FILE, 'w') as f:
            f.write(udev_content)

        sriov_config.del_udev_rule_for_legacy_sriov_pf('eth1')

        # Check that eth1 rule is removed but eth2 rule remains
        with open(sriov_config._UDEV_LEGACY_RULE_FILE, 'r') as f:
            content = f.read()

        self.assertNotIn('eth1', content)
        self.assertIn('eth2', content)
        self.assertIn('# Comment line', content)

    def test_del_udev_rule_for_legacy_sriov_pf_empty_file(self):
        def fake_reload_udev():
            pass
        self.stub_out('os_net_config.sriov_config.reload_udev_rules',
                      fake_reload_udev)

        # Create a file with only the rule we're removing
        udev_content = '''# Comment line
KERNEL=="eth1", RUN+="/bin/os-net-config-sriov -n %k:5"
'''
        with open(sriov_config._UDEV_LEGACY_RULE_FILE, 'w') as f:
        ...
        # File should be removed since only comments remain
        self.assertFalse(os.path.exists(sriov_config._UDEV_LEGACY_RULE_FILE))

    def test_reset_sriov_pf_legacy_mode(self):
        def fake_del_udev_rule(pf_name):
            pass
        self.stub_out('os_net_config.sriov_config.'
                      'del_udev_rule_for_legacy_sriov_pf',
                      fake_del_udev_rule)

        # Create sysfs structure
        pf_path = os.path.join(common.SYS_CLASS_NET, 'eth1', 'device')
        os.makedirs(pf_path)
        sriov_numvfs_path = os.path.join(pf_path, 'sriov_numvfs')
        with open(sriov_numvfs_path, 'w') as f:
            f.write('5')

        # Create sriov map with legacy PF including all possible elements
        sriov_map = [
            {
                'device_type': 'pf',
                'name': 'eth1',
        ...
        sriov_config.reset_sriov_pf('eth1')

        # Check that numvfs was reset to 0
        with open(sriov_numvfs_path, 'r') as f:
            numvfs = f.read().strip()
        self.assertEqual('0', numvfs)

    def test_reset_sriov_pf_switchdev_mode(self):
        def fake_del_udev_rule(pf_name):
            pass
        self.stub_out('os_net_config.sriov_config.'
                      'del_udev_rule_for_legacy_sriov_pf',
        ...
        # Create sriov map with switchdev PF including all possible elements
        sriov_map = [
            {
                'device_type': 'pf',
                'name': 'eth1',
        ...
        # Should not raise any exception, but also should not reset
        sriov_config.reset_sriov_pf('eth1')

    def test_reset_sriov_pf_io_error(self):
        def fake_del_udev_rule(pf_name):
            pass
        self.stub_out('os_net_config.sriov_config.'
                      'del_udev_rule_for_legacy_sriov_pf',
        ...
        # Create sriov map but no sysfs path (will cause IOError)
        sriov_map = [
            {'device_type': 'pf', 'name': 'eth1', 'link_mode': 'legacy',
             'numvfs': 5}
        ]
        common.write_yaml_config(common.SRIOV_CONFIG_FILE, sriov_map)
