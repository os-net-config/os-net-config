...
_V6_IFCFG_WITH_DEFAULTGW = _BASE_IFCFG + """IPV6INIT=yes
IPV6_AUTOCONF=no
IPV6_SET_SYSCTLS=yes
IPV6_FORCE_ACCEPT_RA=no
IPV6ADDR=2001:abc:a::/64
IPV6_DEFAULTGW=2001:abc:a::1
...
_V6_IFCFG_EXPANDED = _BASE_IFCFG + """IPV6INIT=yes
IPV6_AUTOCONF=no
IPV6_SET_SYSCTLS=yes
IPV6_FORCE_ACCEPT_RA=no
IPV6ADDR=2620:00cf:00cf:aaaa:0000:0000:0000:0064/64
IPV6ADDR_SECONDARIES="2620:00cf:00cf:bbbb:0000:0000:0000:0064/64"
DNS1=2620:00cf:00cf:aaaa:0000:0000:0000:0001
"""

_V6_IFCFG_COMPRESSED = _BASE_IFCFG + """IPV6INIT=yes
IPV6_AUTOCONF=no
IPV6_SET_SYSCTLS=yes
IPV6_FORCE_ACCEPT_RA=no
IPV6ADDR=2620:cf:cf:aaaa::64/64
IPV6ADDR_SECONDARIES="2620:cf:cf:bbbb::64/64"
DNS1=2620:cf:cf:aaaa::1
"""
...
172.22.0.0/24 via 192.168.1.1 dev em1 table 200 metric 10000
"""
...
_LINUX_BOND_WITH_OVS_EXTRA = """# This file is autogenerated by os-net-config
DEVICE=bond0
ONBOOT=yes
HOTPLUG=no
NM_CONTROLLED=no
BOOTPROTO=dhcp
OVS_EXTRA="set port bond0 other_config:stp-path-cost=2"
"""

_LINUX_BOND_WITH_OVS_EXTRA_MULTI = """# This file is autogenerated by os-net-config
DEVICE=bond0
ONBOOT=yes
HOTPLUG=no
NM_CONTROLLED=no
BOOTPROTO=dhcp
OVS_EXTRA="set port bond0 other_config:stp-path-cost=2 -- \
set port bond0 other_config:stp-port-priority=32"
"""
...
class TestIfcfgNetConfig(base.TestCase):
    ...
    def test_add_route_with_table(self):
        ...
        route3 = objects.Route('192.168.1.1', '172.21.0.0/24', False,
                               route_table=200)
        # Test route with standalone metric parameter (currently not supported)
        route4 = objects.Route('192.168.1.1', '172.22.0.0/24', False,
                               metric=10000, route_table=200)
    ...
    def test_network_with_routes_standalone_metric(self):
        """Test routes with standalone metric field (not in route_options).

        This test demonstrates that the standalone metric parameter is not
        currently supported by the ifcfg provider. The metric field should
        be honored but is currently ignored.
        """
        route1 = objects.Route('192.168.1.1', default=True, metric=10)
        route2 = objects.Route('192.168.1.1', '172.19.0.0/24')
        route3 = objects.Route('2001:db8:a::1', default=True, metric=200)
        v4_addr = objects.Address('192.168.1.2/24')
        v6_addr = objects.Address('2001:db8:a::10/64')
        interface = objects.Interface('em1', addresses=[v4_addr, v6_addr],
                                      routes=[route1, route2, route3])
        ...
        expec_routes4 = """default via 192.168.1.1 dev em1 metric 10
172.19.0.0/24 via 192.168.1.1 dev em1
"""
        expec_routes6 = """default via 2001:db8:a::1 dev em1 metric 200
"""
        actual_routes = self.get_route_config()
        self.assertEqual(expec_routes4, actual_routes)
        actual_routes6 = self.get_route6_config()
        self.assertEqual(expec_routes6, actual_routes6)

    def test_network_with_ipv6_routes_standalone_metric(self):
        """Test IPv6 routes with standalone metric field (not in route_options)

        This test demonstrates that the standalone metric parameter is not
        currently supported by the ifcfg provider for IPv6 routes.
        """
        route1 = objects.Route('2001:db8::1', '2001:db8::0/64', metric=3000)
        route2 = objects.Route('fd00:fd00:2000::1',
                               '2001:db8:dead:beef:cafe::/56',
                               metric=200)
        v6_addr = objects.Address('2001:db8:a::10/64')
        interface = objects.Interface('em1', addresses=[v6_addr],
        ...
        expec_routes6 = """2001:db8::0/64 via 2001:db8::1 dev em1 metric 3000
2001:db8:dead:beef:cafe::/56 via fd00:fd00:2000::1 dev em1 metric 200
"""
        actual_routes6 = self.get_route6_config()
        self.assertEqual(expec_routes6, actual_routes6)
...
"""
    ...
    def test_linux_bond_with_ovs_extra_valid(self):
        """Test LinuxBond with ovs_extra when used as OVS bridge member"""
        interface1 = objects.Interface('em1')
        interface2 = objects.Interface('em2')
        bond = objects.LinuxBond('bond0', use_dhcp=True,
                                 members=[interface1, interface2],
                                 ovs_extra=["set port bond0 "
                                            "other_config:stp-path-cost=2"])

        # For ifcfg, we need to simulate the bridge_name being set
        # This would normally happen when the bond is added to an OVS bridge
        bond.bridge_name = 'br0'
        ...
        self.assertEqual(_LINUX_BOND_WITH_OVS_EXTRA,
                         self.get_linux_bond_config('bond0'))

    def test_linux_bond_with_ovs_extra_invalid(self):
        """Test LinuxBond with ovs_extra when NOT used as OVS bridge member"""
        interface1 = objects.Interface('em1')
        interface2 = objects.Interface('em2')
        bond = objects.LinuxBond('bond0', use_dhcp=True,
                                 members=[interface1, interface2],
        ...
        # Add bond directly without being a bridge member (no bridge_name set)
        # This should raise an InvalidConfigException
        self.assertRaises(
            objects.InvalidConfigException,
            self.provider.add_linux_bond,
            bond
        )

    def test_linux_bond_with_ovs_extra_multiple_commands(self):
        """Test LinuxBond with multiple ovs_extra commands"""
        interface1 = objects.Interface('em1')
        interface2 = objects.Interface('em2')
        bond = objects.LinuxBond('bond0', use_dhcp=True,
                                 members=[interface1, interface2],
                                 ovs_extra=[
                                     ...
                                     "other_config:stp-path-cost=2",
                                     "set port bond0 "
                                     "other_config:stp-port-priority=32"
                                 ])
...
"""
    ...
    def test_get_dpdk_pci_addresses_multiple(self):
        content = (
            '# This file is autogenerated by os-net-config\n'
            'DEVICE=dpdkbond0\n'
            'DEVICETYPE=ovs\n'
            'TYPE=OVSDPDKBond\n'
            'OVS_EXTRA="set Interface dpdk0 options:dpdk-devargs=0000:19:06.3 '
            '-- set Interface dpdk1 options:dpdk-devargs=0000:19:0a.3 '
            '-- set Interface dpdk0 mtu_request=$MTU '
            '-- set Interface dpdk1 mtu_request=$MTU '
            '-- set Interface dpdk0 options:n_rxq=$RX_QUEUE '
            '-- set Interface dpdk1 options:n_rxq=$RX_QUEUE"\n'
        ...
        try:
            ...
            pci_list = self.provider._get_dpdk_pci_addresses(tmp.name)
            self.assertIn('0000:19:06.3', pci_list)
            self.assertIn('0000:19:0a.3', pci_list)
            self.assertEqual(2, len(pci_list))
        finally:
            if os.path.exists(tmp.name):
                os.remove(tmp.name)

    def test_get_dpdk_port_pci_address_single_quote(self):
        content = (
            'DEVICE=dpdk0\n'
            'TYPE=OVSDPDKPort\n'
            "OVS_EXTRA='set Interface dpdk0 options:dpdk-devargs=0000:19:06.3'"
        ...
        try:
            ...
            pci_address = self.provider._get_dpdk_pci_addresses(tmp.name)
            self.assertEqual(['0000:19:06.3'], pci_address)
        finally:
            if os.path.exists(tmp.name):
                os.remove(tmp.name)

    def test_get_dpdk_port_pci_address_none(self):
        content = (
            '# No OVS_EXTRA line here\n'
            'DEVICE=dpdk0\n'
            'TYPE=OVSDPDKPort\n'
        )
        tmp = tempfile.NamedTemporaryFile('w', delete=False)
        try:
            ...
            pci_address = self.provider._get_dpdk_pci_addresses(tmp.name)
            self.assertEqual([], pci_address)
        finally:
            if os.path.exists(tmp.name):
                os.remove(tmp.name)

    def test_get_dpdk_bond_pci_addresses_none(self):
        content = (
            '# No OVS_EXTRA line here\n'
            'DEVICE=dpdkbond0\n'
            'TYPE=OVSDPDKBond\n'
        ...
        try:
            ...
            pci_list = self.provider._get_dpdk_pci_addresses(tmp.name)
            self.assertEqual([], pci_list)
        finally:
            if os.path.exists(tmp.name):
                os.remove(tmp.name)

    def test_get_dpdk_pci_addresses_max_valid(self):
        content = (
            'DEVICE=dpdk0\n'
            'TYPE=OVSDPDKPort\n'
            'OVS_EXTRA="set Interface dpdk0 '
            'options:dpdk-devargs=FFFF:FF:1F.7"\n'
        )
        tmp = tempfile.NamedTemporaryFile('w', delete=False)
        try:
            tmp.write(content)
            ...
            pci_address = self.provider._get_dpdk_pci_addresses(tmp.name)
            self.assertEqual(['FFFF:FF:1F.7'], pci_address)
        finally:
            if os.path.exists(tmp.name):
                os.remove(tmp.name)

    def test_get_dpdk_pci_addresses_invalid_device_function(self):
        # Test with invalid device (20 > 1F) and function (8 > 7)
        content = (
            'DEVICE=dpdk0\n'
            'TYPE=OVSDPDKPort\n'
            'OVS_EXTRA="set Interface dpdk0 '
            'options:dpdk-devargs=0000:03:20.8"\n'
        )
        tmp = tempfile.NamedTemporaryFile('w', delete=False)
        try:
            tmp.write(content)
            ...
            # Invalid PCI addresses should still be extracted as the regex
            # currently doesn't validate ranges, only format
            pci_address = self.provider._get_dpdk_pci_addresses(tmp.name)
            self.assertNotIn('0000:03:20.8', pci_address)
        finally:
            if os.path.exists(tmp.name):
                os.remove(tmp.name)

    def test_get_dpdk_pci_addresses_mixed_valid_invalid(self):
        # Test bond with mix of valid and invalid PCI addresses
        content = (
            'DEVICE=dpdkbond0\n'
            'TYPE=OVSDPDKBond\n'
            'OVS_EXTRA="set Interface dpdk0 options:dpdk-devargs=0000:19:06.3 '
            '-- set Interface dpdk1 options:dpdk-devargs=0000:19:FF.F"\n'
        )
        tmp = tempfile.NamedTemporaryFile('w', delete=False)
        try:
            tmp.write(content)
            ...
            pci_list = self.provider._get_dpdk_pci_addresses(tmp.name)
            self.assertIn('0000:19:06.3', pci_list)  # Valid
            self.assertNotIn('0000:19:FF.F', pci_list)  # Invalid
            self.assertEqual(1, len(pci_list))
        finally:
            if os.path.exists(tmp.name):
                os.remove(tmp.name)
...
class TestIfcfgNetConfigApply(base.TestCase):
    ...
    def test_change_restart_required(self):
        ...
        # Removing IPV6_DEFAULTGW/IPV6_DEFAULTDEV (generated by older code)
        # should not require restart; kept in permitted_changes for backwards
        # compatibility even though new code no longer generates them.
        file = open(interface_filename, 'w')
        file.write(_V6_IFCFG_WITH_DEFAULTGW)
        file.close()
        self.assertFalse(self.provider.ifcfg_requires_restart(
                         interface_filename, _V6_IFCFG))
    ...
    def test_ipv6_format_diff_no_change(self):
        ...
        tmpdir = tempfile.mkdtemp()
        filename = tmpdir + '/ifcfg-em1'

        file = open(filename, 'w')
        file.write(_V6_IFCFG_EXPANDED)
        file.close()

        self.assertFalse(utils.diff(filename, _V6_IFCFG_COMPRESSED))
