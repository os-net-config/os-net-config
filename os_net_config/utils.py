...
def delete_sriov_entries_for_pf(pfname):
    """Remove all VF and PF entries for a given PF device from the SR-IOV map.
    If the resulting map is empty, remove the SRIOV_CONFIG_FILE."""
    sriov_map = common.get_sriov_map()
    new_map = []
    for item in sriov_map:
        if item["device_type"] == "vf" and item["device"].get("name") == pfname:
            continue  # Skip this VF
        if item["device_type"] == "pf" and item.get("name") == pfname:
